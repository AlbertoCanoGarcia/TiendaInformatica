Public Class VentanaAlbaran
    ...
    Private Sub botoninsertarfila_Click(sender As Object, e As EventArgs) Handles botoninsertarfila.Click
        ...
        Else
            ...
            Else
                ...
                a = CSng(clase.RemplazarComa(textounidad.Text))
            End If
    ...
    Private Sub rellenar()
        textonumeroalbaran.Text = Format(ModuloEmpresa.contadorAlbaran + 1, "0000")
        Dim fechas As Date
    ...
    Private Sub textodatobuscar_SelectedIndexChanged(sender As Object, e As EventArgs) Handles textodatobuscar.SelectedIndexChanged
        ...
        ElseIf (criterio.Equals("id")) Then
            orden_sql = "Select nombre, nif_cif,direccion,localidad,telefono, Email,Descuento,id from clientes where id='" + busqueda + "'"
        Else
        ...
        codcliente = Midatareader(7).ToString
        Midatareader.Close()
    ...
    Private Sub textodescripcion_KeyUp(sender As Object, e As KeyEventArgs) Handles textodescripcion.KeyUp
        actualizarCombobox("Articulos", textodescripcion.Text, "Descripcion", textodescripcion)
    End Sub
    ...
    Private Sub botonaceptar_Click(sender As Object, e As EventArgs) Handles botonaceptar.Click
        ...
        Try
            orden_sql = "insert into Ventas(NVenta,Fecha,DireccionDeEntrega,CodCliente,Nombre,Direccion,FormaDePago,Observaciones,SumaImporte,PorcentajeDescuento,ImporteDescuento,Portes,BaseImponible,PorcentajeIVA,ImporteIVA,TotalAlbaran) " &
            ...
            '    Midatareader = comando.ExecuteReader
            '    Midatareader.Read()
            '    Dim stock As Single = CSng((clase.RemplazarComa(Midatareader(0).ToString))) - a
            '    Midatareader.Close()
            ...
            While (x < numlinea)
                ...
                orden_sql = "insert into LineasVenta(NAlbaran,Nlinea,Codprod,Descripcion,PVP,Unidad,PorcentajeDescuento,ImporteLinea) " &
                        "VALUES('" & textonumeroalbaran.Text &
                        "','" & Format(x + 1, "0000") &
                        "','" & codprod2(x) &
                        "','" & descrip(x) &
        ...
        Try
            orden_sql = "update Empresa set ContadorAlbaran= '" & ModuloEmpresa.contadorAlbaran + 1 &
                "' where Nombre =  '" & "CanoMatic" & "';"
            comando = New SqlClient.SqlCommand(orden_sql, Miconexion)
            comando.ExecuteNonQuery()

            orden_sql = "Select Total_gastado_ano from Clientes where id='" & codcliente & "' "
            comando = New SqlClient.SqlCommand(orden_sql, Miconexion)
            Midatareader = comando.ExecuteReader
            Midatareader.Read()
            ...
            orden_sql = "update Clientes set Fecha_ultima_venta= '" & Format(fecha, "MM/dd/yyyy") &
           "' ,Total_gastado_ano='" & clase.RemplazarComa(totalgastadoaño) &
                "' where ID = '" & codcliente & "';"
    ...
    Private Sub botonfacturadirecta_Click(sender As Object, e As EventArgs) Handles botonfacturadirecta.Click
        ...
        If (facturado = 0) Then
            ...
            Try
                orden_sql = "insert into Facturas(Nfactura,Fecha,DireccionDeEntrega,CodCliente,Nombre,Direccion,FormaDePago,Observaciones,SumaImporte,PorcentajeDescuento,ImporteDescuento,Portes,BaseImponible,PorcentajeIVA,ImporteIVA,TotalAlbaran,NAlbaran) " &
    ...
    Private Sub botonimprimir_Click(sender As Object, e As EventArgs) Handles botonimprimir.Click
        ...
        Dim adaptador As New SqlClient.SqlDataAdapter("select *  from LineasVenta ", Miconexion)

        adaptador.Fill(ds, "LineasVenta")
        informe.SetDataSource(ds)
    ...
    Private Sub textounidad_KeyUp(sender As Object, e As KeyEventArgs) Handles textounidad.KeyUp
        ...
        End If
        textoimportelinea.Text = clase.RemplazarComa(Format(CSng(pvp * cantidad * ((100 - descuento) / 100)), "#,##0.00"))
    End Sub
